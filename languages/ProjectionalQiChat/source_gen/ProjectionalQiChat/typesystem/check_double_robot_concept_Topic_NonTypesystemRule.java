...
public class check_double_robot_concept_Topic_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  public check_double_robot_concept_Topic_NonTypesystemRule() {
  }
  public void applyRule(final SNode topic, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    ...
    for (SNode node_HumanConcept_ : SNodeOperations.ofConcept(SLinkOperations.getChildren(SLinkOperations.getTarget(topic, LINKS.header$Q5v0), LINKS.localConcepts$hTKt), CONCEPTS.RobotConcept$rd)) {
      if (SetSequence.fromSet(conceptNames).contains(SPropertyOperations.getString(node_HumanConcept_, PROPS.name$MnvL))) {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
          IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(node_HumanConcept_, "Double local robot concept name:  " + SPropertyOperations.getString(node_HumanConcept_, PROPS.name$MnvL), "r:2e52173d-a6ef-4c74-b7ba-dc1accb9a614(ProjectionalQiChat.typesystem)", "7237576769905744950", null, errorTarget);
          {
            BaseQuickFixProvider intentionProvider = new BaseQuickFixProvider("ProjectionalQiChat.typesystem.change_names_Topic_QuickFix", "7237576769906183287", false);
